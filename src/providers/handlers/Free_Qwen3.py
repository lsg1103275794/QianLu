"""
Qwen3-30B-A3B API handler (OpenAI-compatible).
"""
import json
import aiohttp
...
class FreeQwen3Handler(BaseAPIHandler):
    """Template class for handling OpenAI-compatible API interactions.
    
    Note: This is an auto-generated handler for Qwen3-30B-A3B.
    ...
    **This template now expects flattened configuration based on environment variables.**
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.provider_name = config.get('provider_name', 'Free_Qwen3') 
    ...
    async def _make_request(self, endpoint_path: str, payload: Dict[str, Any], method: str = "POST") -> Dict[str, Any]:
        ...
        logger.debug(f"Sending {method} request to endpoint: {request_url} for provider {self.provider_name}. Effective timeout: {self.request_timeout}s")
        try:
            async with aiohttp.ClientSession() as session:
