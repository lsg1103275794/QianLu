"""
Mistral AI API handler implementation (OpenAI Compatible).
"""
import json
import aiohttp
...
class MistralAiHandler(BaseAPIHandler):
    """Handles interaction with Mistral AI API (OpenAI Compatible)."""

    def __init__(self, config: Dict[str, Any]):
        super().__init__(config)
        self.provider_name = config.get('provider_name', 'mistral_ai')
        self.api_key = config.get('api_key')
        self.endpoint = config.get('endpoint', 'https://api.mistral.ai/v1') # Default Mistral endpoint
        ...
        logger.info(f"Mistral AI Handler Initialized: Name='{self.provider_name}', Endpoint='{self.endpoint}', DefaultModel='{self.default_model}'")
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(self.models_endpoint, headers=headers, timeout=10) as response:
                    if response.status != 200:
                        ...
                        logger.error(f"Mistral AI API error fetching models: {response.status}, {error_text}")
                    ...
                    logger.info(f"Available Mistral AI models: {models}")
                    return models
        except aiohttp.ClientError as e:
            logger.error(f"Mistral AI API connection error fetching models: {e}")
            raise APIError(f"Connection error: {e}")
        except Exception as e:
            logger.error(f"Error getting Mistral AI models: {str(e)}")
    ...
    async def _make_request(self, headers: Dict[str, str], payload: Dict[str, Any]) -> Dict[str, Any]:
        logger.debug(f"Sending request to Mistral AI endpoint: {self.chat_endpoint}")
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if response.status != 200:
                        error_text = await response.text()
                        logger.error(f"Mistral AI API HTTP error: status={response.status}, response: '{error_text[:200]}...'")
                        raise APIResponseError(f"HTTP {response.status}: {error_text}")
                    return await response.json()
        except aiohttp.ClientError as e:
            logger.error(f"Mistral AI API request failed (network error): {e}")
            raise APIError(str(e))
        except json.JSONDecodeError as e:
            logger.error(f"Failed to decode JSON response from Mistral AI: {e}")
    ...
    async def generate(
        ...
        **kwargs
        ...
        if not target_model:
            raise ConfigError("No model specified and no default model configured for Mistral AI")
        ...
        try:
            ...
            logger.warning(f"Mistral AI response format unexpected: {result}")
            raise APIResponseError("Unexpected response format from Mistral AI")
        except RetryError as e:
            logger.critical(f"Mistral AI API request failed after retries: {e}")
                ...
                async with session.post(
                    ...
                    async for line in response.content:
                        ...
                        if line_str.startswith("data: "):
                            ...
                            if data_str == "[DONE]":
                                logger.info("Mistral AI Stream chat completed")
                                break
                            try:
                                data = json.loads(data_str)
                                yield data 
                            except json.JSONDecodeError as e:
                                logger.error(f"Failed to parse Mistral AI stream response: {e}, data: '{data_str}'")
        except Exception as e:
            logger.error(f"Mistral AI stream chat error: {str(e)}")
            yield {"error": str(e)} 