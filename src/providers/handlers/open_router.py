"""
open_router API handler (OpenAI-compatible).
"""
import json
import aiohttp
...
class OpenRouterHandler(BaseAPIHandler):
    ...
    Note: This is an auto-generated handler for open_router.
    ...
    **This template now expects flattened configuration based on environment variables.**
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    ...
                    if not models and self.default_model:
                        logger.warning(f"No models returned from API, fallback to default_model: {self.default_model}")
    ...
    async def generate_text(self, prompt: str, model: Optional[str] = None, **kwargs) -> str:
        ...
        # 合理处理 stop 参数，优先用用户自定义，否则用默认
        stop_words = final_api_params.get("stop")
        if not stop_words:
            stop_words = ["\n", "。", "！", "?", ""]
        payload = {
            "model": target_model,
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post( 
                    ...
                    # 修复：与 volc_engine 一致，直接 yield chunk
                    async for line in response.content:
                        ...
                        try:
                            ...
                            elif "error" in chunk and "message" in chunk["error"]:
                                # 兼容 OpenAI error 格式，前端可弹窗
                                yield {
                                    "error": chunk["error"]["message"],
                                    "code": chunk["error"].get("code", None)
                                }
                                logger.warning(f"Yielded error chunk to frontend: {chunk['error']['message']}")
                                break  # 出错后直接结束流
                            else:
                                logger.warning(f"Unknown or empty stream chunk format, skipping: {chunk}")
                                continue
                        except json.JSONDecodeError as e:
