...
class TemplateOpenAIHandler(BaseAPIHandler):
    ...
    **This template now expects flattened configuration based on environment variables.**
    ...
    async def generate(self, prompt: str, model: Optional[str] = None, **kwargs) -> str:
        ...
        return await self.generate_text(prompt, (model or None).strip() if model else None, **kwargs)
        ...
        try:
            ...
            if "choices" in result and isinstance(result["choices"], list) and len(result["choices"]) > 0:
                ...
                if content:
                    ...
                    if usage:
                        logger.info(f"OpenAI Compatible Template API call successful. Usage: {usage}")
                    else:
                        logger.info("OpenAI Compatible Template API call successful.")
