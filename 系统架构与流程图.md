...
graph TB
    subgraph "开发机器"
        ...
        subgraph "本地存储"
            DB[(SQLite数据库<br/>data/QianLu_data.db)]
...
graph TB
    subgraph "Docker Host"
        subgraph "应用容器"
            App[QianLu-app<br/>Python 3.12<br/>FastAPI + Frontend]
        end
...
graph TB
    ...
    subgraph "结果存储"
        Result[(结果数据库)]
        Notify[通知系统<br/>WebSocket]
    end
    
    Request[API请求] --> Check{任务类型}
    Check -->|同步| Sync
    Check -->|异步| Queue
